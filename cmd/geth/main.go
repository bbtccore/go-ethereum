...
const (
	clientIdentifier = "ferrox-geth" // Client identifier to advertise over the network
)
...
var app = flags.NewApp("the FERROX (FRX) command line interface")
...
func prepare(ctx *cli.Context) {
	...
	case !ctx.IsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Geth on " + params.FerroxMainnetName + "...")
	}
	...
	if !ctx.IsSet(utils.CacheFlag.Name) && !ctx.IsSet(utils.NetworkIdFlag.Name) {
		...
		if !ctx.IsSet(utils.HoleskyFlag.Name) &&
			...
			log.Info("Bumping default cache on "+params.FerroxMainnetName, "provided", ctx.Int(utils.CacheFlag.Name), "updated", 4096)
