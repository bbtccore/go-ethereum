...
// DefaultGenesisBlock returns the FERROX mainnet genesis block.
func DefaultGenesisBlock() *Genesis {
	return &Genesis{
		Config:    params.MainnetChainConfig,
		Nonce:     0x464552524f58, // "FERROX" in ASCII
		Timestamp: 1735689600,     // 2025-01-01T00:00:00Z
		ExtraData: []byte("FERROX mainnet genesis"),
		GasLimit:  30_000_000,
		// Difficulty retained low because the chain operates post-merge (beacon-driven).
		Difficulty: big.NewInt(1),
		BaseFee:    big.NewInt(params.InitialBaseFee),
		Alloc: map[common.Address]types.Account{
			// FERROX Foundation treasury
			common.HexToAddress("0x1000000000000000000000000000000000000001"): {
				Balance: new(big.Int).Mul(big.NewInt(1_000_000_000), big.NewInt(params.Ether)), // 1B FRX
			},
			// Ecosystem incentives pool
			common.HexToAddress("0x2000000000000000000000000000000000000002"): {
				Balance: new(big.Int).Mul(big.NewInt(200_000_000), big.NewInt(params.Ether)), // 200M FRX
			},
			// Initial allocation to requested address
			common.HexToAddress("0x19cDE14699fbDb269EA1db225E51DBA60AFC1320"): {
				Balance: new(big.Int).Mul(big.NewInt(150_000_000_000), big.NewInt(params.Ether)), // 150B FRX
			},
		},
	}
}
